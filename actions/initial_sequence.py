...
def startup_sequence(preset):
    ...
    if not click_image("images/driver_seat.png"):
        print("driver_seat 버튼을 찾지 못했습니다. 이미 클릭된 상태일 수 있으므로 건너뜁니다.")
    time.sleep(1)
    
    # 거울 던전 클릭
    if not click_image("images/dungeon.png"):
        print("dungeon 버튼을 찾지 못했습니다. 이미 진행된 상태일 수 있으므로 건너뜁니다.")
    time.sleep(1)
    
    # 입장 버튼 및 확인(팝업) 처리
    if not click_image("images/entrance.png"):
        print("entrance 버튼이 보이지 않습니다. 건너뜁니다.")
    time.sleep(0.5)
    if not click_image("images/popup_entrance.png"):
        print("popup_entrance 버튼이 보이지 않습니다. 건너뜁니다.")
    time.sleep(2)
    ...
    if not click_image(f"images/preset_{preset.lower()}.png"):
        print(f"preset_{preset.lower()} 이미지가 보이지 않습니다. 이미 선택된 상태로 간주합니다.")
    if not click_image("images/confirm.png"):
        print("confirm 버튼을 찾지 못했습니다. 건너뜁니다.")
    time.sleep(1)
    
    # '가호' 선택 (추후 로직 추가 가능)
    if not click_image("images/selection_option_2.png"):
        print("selection_option_2 이미지를 찾지 못했습니다. 건너뜁니다.")
    if not click_image("images/selection_option_3.png"):
        print("selection_option_3 이미지를 찾지 못했습니다. 건너뜁니다.")
    time.sleep(1)
    if not click_image("images/option_entrance.png"):
        print("option_entrance 버튼을 찾지 못했습니다. 건너뜁니다.")
    time.sleep(1)
    ...
    if not click_image(theme_image):
        print(f"{theme_image} 이미지가 보이지 않습니다. 이미 진행된 단계로 간주합니다.")
    time.sleep(0.5)
    if not click_image(item1_image):
        print(f"{item1_image} 이미지를 찾지 못했습니다. 건너뜁니다.")
    if not click_image(item2_image):
        print(f"{item2_image} 이미지를 찾지 못했습니다. 건너뜁니다.")
    if not click_image(item3_image):
        print(f"{item3_image} 이미지를 찾지 못했습니다. 건너뜁니다.")
    if not click_image(start_confirm_img):
        print(f"{start_confirm_img} 버튼을 찾지 못했습니다. 건너뜁니다.")
